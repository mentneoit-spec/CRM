...
import { bulkImportClasses, fetchClasses, clearAdminStatus } from '../../redux/slices/adminSlice';

const BulkClassImportDialog = ({ open, onClose }) => {
    ...
    const handleSubmit = async () => {
        if (!file) return;
        const action = await dispatch(bulkImportClasses({ file, mode }));
        if (!action.error) {
    ...
    return (
        <Dialog open={open} onClose={handleClose} maxWidth="sm" fullWidth>
            <DialogTitle sx={{ fontWeight: 'bold' }}>Bulk Import Classes (CSV)</DialogTitle>
                ...
                <Box sx={{ display: 'flex', gap: 2, alignItems: 'center', mb: 2 }}>
                    <FormControl fullWidth size="small">
                        ...
                        <Select value={mode} label="Mode" onChange={(e) => setMode(e.target.value)}>
                            <MenuItem value="skip">Skip if Class Code/Name exists</MenuItem>
                            <MenuItem value="update">Update if Class Code/Name exists</MenuItem>
                        </Select>
                    </FormControl>
                </Box>
                    ...
                    <Box sx={{ mt: 1 }}>
                        ...
                        {!didWork && hasErrors && (
                            <Typography variant="body2" color="error" sx={{ mb: 1 }}>
                                No classes were imported. Fix the CSV errors and try again.
                            </Typography>
                        )}
...
export default BulkClassImportDialog;
